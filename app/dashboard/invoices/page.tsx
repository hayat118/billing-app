...
const InvoicesPage = () => {
  ...
  const validateForm = () => {
    ...
    if (hasInvalidItems) {
      newErrors.items =
        "Please select a valid medicine, quantity, and price for all rows";
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const currentStockItems = loadStockItems();
    const updatedStockItems = reduceStockForInvoiceItems(
      invoiceItems,
      currentStockItems,
    );
  ...
  const columns = [
    ...
    },
    {
      ...
      title: "Prescribed By",
      render: (value: string) => (
        <div className="text-gray-700 text-sm">{value || "N/A"}</div>
      ),
    },
    {
      key: "total",
    ...
    },
    {
      ...
      render: (value: string) => (
        <span
          className={`px-2 py-0.5 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusColor(value)}`}
        >
          {getStatusText(value)}
        </span>
      ),
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Create pharmacy invoices, track batch details, and manage medical
            billing records.
          </p>
        </div>
        <div>
      ...
      {/* Invoices Table */}
      <Card
        title={`Billing Ledger (${filteredInvoices.length})`}
              ...
              <div className="flex justify-between items-center px-6 py-4 border-b border-gray-200 bg-gray-50">
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-0.5">
                    Generate prescription bill and verify batch numbers
                  </p>
                </div>
                <button
                  onClick={() => setIsModalOpen(false)}
                  ...
                  <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
                      <tr>
                        ...
                        <th className="px-3 py-2.5 text-left text-xs font-semibold uppercase text-gray-500 w-28">
                          Batch No
                        </th>
                        <th className="px-3 py-2.5 text-left text-xs font-semibold uppercase text-gray-500 w-28">
                          Expiry
                        </th>
                        <th className="px-3 py-2.5 text-left text-xs font-semibold uppercase text-gray-500 w-20">
                          Qty
                        </th>
                        <th className="px-3 py-2.5 text-left text-xs font-semibold uppercase text-gray-500 w-24">
                          MRP ($)
                        </th>
                        <th className="px-3 py-2.5 text-left text-xs font-semibold uppercase text-gray-500 w-20">
                          GST %
                        ...
                        <th className="px-3 py-2.5 text-right text-xs font-semibold uppercase text-gray-500 w-24">
                          Total
                        </th>
